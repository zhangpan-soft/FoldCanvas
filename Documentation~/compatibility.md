...
FoldCanvas `1.0.0-rc.1` is a pre-release candidate, and compatibility changes
...
|---|---|---|
| UPM package | `1.0.0-rc.1` | deterministic `.tgz`, checksum, file manifest, and RC evidence |
...
cancellation/retry, resource-envelope, and hosted long-run evidence. M14 adds
release gates and frozen evidence; it does not add or remove a Runtime geometry
operation or change FoldScript `0.1`.
...
`1.0.0-rc.1` freezes the candidate surface but remains a pre-release. Another
RC may fix a release blocker while retaining compatible source/API behavior.
...
is a separate decision from passing M14.

The machine-readable freeze and rollback target live in
[`m14-release-candidate.json`](m14-release-candidate.json). Installation and
troubleshooting are documented in the
[`release-candidate guide`](release-candidate.md).

## 中文摘要

FoldCanvas 当前为 `1.0.0-rc.1` 发布候选，不会把破坏性变化藏在普通提交里。公共 Runtime API
