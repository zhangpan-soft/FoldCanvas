...
FoldCanvas `1.0.0-rc.2` is a pre-release candidate, and compatibility changes
...
|---|---|---|
| UPM package | `1.0.0-rc.2` | deterministic `.tgz`, checksum, file manifest, and public-release evidence |
...
The RC carries forward replayable Editor-only robustness, scale,
cancellation/retry, resource-envelope, and hosted long-run evidence. M15 adds
public asset and source-upgrade gates; it does not add or remove a Runtime
geometry operation or change FoldScript `0.1`.
...
`1.0.0-rc.2` preserves the candidate surface but remains a pre-release. RC1 is
an immutable prior release; it is not rebuilt after M15 package content changes.
Another RC may fix a release blocker while retaining compatible source/API behavior.
After stable `1.0.0`, a patch keeps the public API and compatible behavior, a
minor version may add backward-compatible API, and a major version is required
for a removal, changed signature, or incompatible behavior. Stable publication
is a separate decision from passing M15.

The historical RC1 freeze lives in
[`m14-release-candidate.json`](m14-release-candidate.json); the current public
distribution, upgrade, and stable-exit gates live in
[`m15-public-distribution.json`](m15-public-distribution.json). Installation
and troubleshooting are documented in the
[`release-candidate guide`](release-candidate.md).

## 中文摘要

FoldCanvas 当前为 `1.0.0-rc.2` 发布候选，不会把破坏性变化藏在普通提交里。公共 Runtime API
