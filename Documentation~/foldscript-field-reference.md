...
> **Implementation status:** M09 implements FoldScript `0.1` as executable,
> bounded Runtime import/export over the M00-M09 source model. It supports
> planar `rectangle` and `disk`/ellipse compilation,
> `rigidTransform`, edge-aligned rigid-crease `fold`, rectangle `roll`,
> explicit rectangle `sphericalWrap` and `toroidalWrap`, optional normalized
> boundary-reference spans, deterministic `weld`/`bridge` Stitch, and
> `solidify` through the Unity
...
It may select one non-wrapping normalized sub-chain:

```json
{ "panel": "wall", "boundary": "vMax", "span": [0.0, 0.125] }
```
...
| `span` | `[number,number]` | Optional normalized current-space arc-length interval in authored boundary order. It must satisfy `0 <= startT < endT <= 1`. Omission selects the complete boundary. |

M09 spans are finite, normalized, and non-wrapping. Selection occurs before
`reverseB`; when either endpoint uses a span, both correspondence paths are
treated as open chains. An endpoint between authored samples is inserted by
splitting the actual boundary-adjacent source triangle, with source position,
UV, ownership, and provenance interpolated deterministically. It is never a
free-floating render-only point. A failed Stitch rolls back these insertions.
...
### 6.5 `toroidalWrap` — implemented in M09

`toroidalWrap` maps one explicit rectangular 2D parameter panel into a toroidal
patch in that panel's current congruent planar frame. It is a deformation rule,
not a torus primitive, topology shortcut, or automatic weld.

| Field | Type | Required | Meaning |
|---|---:|:---:|---|
| `panel` | ID | yes | Rectangle parameter panel to map. |
| `majorRadius` | positive number | yes | Distance `R` from the resolved origin to the tube centerline, in document units. M09 requires `R > r`. |
| `minorRadius` | positive number | yes | Tube radius `r`, in document units. M09 requires `0 < r < R`. |
| `majorAngleRange` | `[number,number]` | yes | Signed centerline-angle endpoints in degrees; the non-zero span may not exceed one turn. |
| `minorAngleRange` | `[number,number]` | yes | Signed tube-profile-angle endpoints in degrees; the non-zero span may not exceed one turn. |
| `wrapDirection` | enum | yes | `"majorAlongU"` maps U to the major angle and V to the minor angle; `"majorAlongV"` swaps them. |

Before mapping, the complete target must remain a finite, non-degenerate,
congruent planar rectangle embedding. The compiler preserves prior translation,
rotation, and unit reflection while rejecting metric-changing scale, shear,
collapse, or non-planarity with `FC8008 UnsupportedToroidalEmbedding`.

Let `a` be the major angle, `b` the minor angle, and resolve:
...
Triangle indices are normalized and then checked so their geometric normals
face `outward(a,b)`. Positive and negative signed ranges preserve their
authored parameter direction; the result never relies on a two-sided material.
Each full-turn axis requires at least three source segments. M09 rejects
multi-turn ranges and horn/spindle radii instead of emitting overlapping or
self-intersecting toroidal layers.

An exact full angle makes the corresponding minimum and maximum boundary
positions coincide, but their render vertices and source UVs remain distinct.
Only a declared seam selected by `stitch` may union their
`TopologyVertexId`s. A closed torus therefore uses two explicit Weld seams,
one for each parameter cycle, and validates to one component with Euler
characteristic `0`.

中文摘要：`toroidalWrap` 只把一个明确的二维矩形参数面板映射到当前局部环面，
不会调用 Unity 环面原语，也不会因为位置重合就自动焊接。U/V 两个周期必须分别
通过显式 Seam 与 `stitch` 关闭；UV 接缝两侧渲染顶点仍保留各自二维来源。
...
`rigidTransform`, `fold`, `roll`, `sphericalWrap`, or `toroidalWrap` cannot
target any panel selected by an earlier Stitch. It returns `FC2010` and no Mesh. Solidify is
...
### 6.7 `solidify` — implemented in M04
