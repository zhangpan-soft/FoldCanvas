...
> **暂定项目名，1.0 发布候选。** 一个面向 Unity 的“二维优先、确定性编译”三维曲面引擎。
...
M14 已把验收过的 M00-M13 行为发布为不可变的 `1.0.0-rc.1`。M15 在不改几何
和 FoldScript 的前提下把包推进到 `1.0.0-rc.2`，只承诺真实跑过完整证据的 Unity
`6000.3.20f1`，并新增 GitHub 公共发布资产校验、两个只使用公共 `.tgz` 的全新
Unity 消费工程、从二维源重新编译的升级证明，以及默认阻断的稳定版退出门。
它不是最终 `1.0.0`，也不会自动发布到外部商城。详见
[《M14 发布候选指南》](Documentation~/release-candidate.md)。
...
版本一致，上传 `.tgz`、SHA-256、逐文件清单和 RC 证据；只有与包版本精确匹配的
RC 标签才会作为 GitHub 预发布版本发布，M14 不会发布最终 `1.0.0`。
...
满 168 小时、两次独立定时长跑、零发布阻断 Issue 和精确提交审计才可能变为 ready。
